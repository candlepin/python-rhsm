...
class AsyncBindRequestTests(BaseBindRequest):
    ...
    def test_update_job_status(self):
        ...
        updated_status = self.cp.updateJobStatus(result)
        self.assertTrue(updated_status is not None)
        self.assertTrue('id' in result)
        self.assertEqual(result['id'], updated_status['id'])
        self.assertEqual(result['targetId'], self.consumer_uuid)
        self.assertEqual(updated_status['targetId'], self.consumer_uuid)
        self.assertNotEqual(result['updated'], updated_status['updated'])
    ...
    def _wait_for_job(self, job_id, wait_interval=2, max_attempts=6):
        # If we haven't gotten a job status indicating done by
        # wait_interval*max_attempts, the job could still finish
        # succesfully, but that is slow enough we consider it
        # a failure.
        attempt_count = 0
        ...
        self.fail("Queued job took too long. %s was still not done after "
                  "%s attempts and waiting %s seconds between them." %
                  (job_id, wait_interval, max_attempts))
